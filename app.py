...
def all_mail():
    if request.method == 'POST':
        ...
        msg = Message("[GENERAL] Queries",sender=data,recipients=["adithya2120@gmail.com"])
        msg.body = msg1
        mail.send(msg)
...
def user_mail():
    if request.method == 'POST':
        data = request.form["mail"]
        msg1 = request.form["msg"]
        msg = Message("[BOOKING] related Query",sender=data,recipients=["adithya2120@gmail.com"])
        msg.body = msg1
        mail.send(msg)
...
#--------------x----------------x--------END OF ADMIN AND STAFF PART--------x----------------------x--------------#

 # ------------x----------------x-------------USER DASHBOARD PART----x--------------x-------------------------x-------------------#

                                                #user registration Form
@app.route("/online_data",methods=['POST'])   
def online_data():
    if  request.method == 'POST':
        name = request.form['nam']
        email = request.form['mail']
...
def user_log():
    if request.method == 'POST':
        ...
        return redirect(url_for("userlog"))
                            ...
                            #user part sending license no and location for form
...
def booking(id):
    if "user" in session and session["user"] == True:
        ...
        return redirect(url_for('userlog'))
                            ...
                            #user booking slot after logged in
...
#sending  location to feedback form
